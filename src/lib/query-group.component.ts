...
@Component({
  ...
  imports: [NgTemplateOutlet, UpperCasePipe, QueryConditionComponent],
  template: `
    <div class="qb-group" [class.qb-nested]="depth() > 0" [class.qb-negated]="group().negated">
      <div class="qb-group-header">
        <div class="qb-group-controls">
          @if (conjunctionSelectorTpl(); as tpl) {
            <ng-container
              [ngTemplateOutlet]="tpl"
              [ngTemplateOutletContext]="conjunctionSelectorContext()"
            />
          } @else {
          ...
          @if (negationToggleTpl(); as tpl) {
            <ng-container
              [ngTemplateOutlet]="tpl"
              [ngTemplateOutletContext]="negationToggleContext()"
            />
          } @else {
        ...
        @if (depth() > 0) {
          @if (removeGroupButtonTpl(); as tpl) {
            <ng-container
              [ngTemplateOutlet]="tpl"
              [ngTemplateOutletContext]="removeGroupButtonContext()"
            />
          } @else {
            <button
              type="button"
        ...
        @for (child of group().children; track $index) {
          @if (isCondition(child)) {
            ...
            <qb-group
              ...
              [valueInputTpls]="valueInputTpls()"
              [removeButtonTpl]="removeButtonTpl()"
              [conjunctionSelectorTpl]="conjunctionSelectorTpl()"
              [negationToggleTpl]="negationToggleTpl()"
              [addButtonsTpl]="addButtonsTpl()"
              [removeGroupButtonTpl]="removeGroupButtonTpl()"
      ...
      @if (addButtonsTpl(); as tpl) {
        <ng-container
          [ngTemplateOutlet]="tpl"
          [ngTemplateOutletContext]="addButtonsContext()"
        />
      } @else {
...
export class QueryGroupComponent {
  ...
  // Template inputs - group level
  readonly conjunctionSelectorTpl = input<TemplateRef<ConjunctionSelectorContext>>();
  readonly negationToggleTpl = input<TemplateRef<NegationToggleContext>>();
  readonly addButtonsTpl = input<TemplateRef<AddButtonsContext>>();
  readonly removeGroupButtonTpl = input<TemplateRef<RemoveGroupButtonContext>>();
  ...
  // Template contexts
  protected readonly conjunctionSelectorContext = computed<ConjunctionSelectorContext>(() => ({
    $implicit: this.group().conjunction,
    onChange: (conjunction: Conjunction) => this.onConjunctionChange(conjunction),
  }));

  protected readonly negationToggleContext = computed<NegationToggleContext>(() => ({
    $implicit: this.group().negated,
    onChange: (negated: boolean) => this.updateGroup({ negated }),
  }));

  protected readonly addButtonsContext = computed<AddButtonsContext>(() => ({
    $implicit: undefined,
    onAddCondition: () => this.addCondition(),
    onAddGroup: () => this.addGroup(),
  }));

  protected readonly removeGroupButtonContext = computed<RemoveGroupButtonContext>(() => ({
    $implicit: undefined,
    depth: this.depth(),
